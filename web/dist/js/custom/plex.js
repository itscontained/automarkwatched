...
function getSeries() {
    apiCall("series").done((data) => {
        let s = []
        for (const [rk, series] of Object.entries(data)) {
            s.push({
                Scrobble: JSON.stringify({
                    scrobble: series.scrobble,
                    ratingKey: series.rating_key,
                }),
                RatingKey: series.rating_key.toString(),
                Title: series.title,
                Year: series.year.toString(),
                Library: series.library_uuid,
                Server: series.server_machine_identifier
            })
        }
        TVShowTable.insert(s)
    })
}
